...
if ((isset($_SESSION['user']))){
    ...
    if($polaczenie->connect_errno!=0) {
        ...
        if ($rezultat = @$polaczenie->query($sql)){
            ...
            if($ilu_userow>0) {
                ...
                $id = $_SESSION['id'];
                $rezultat->close();
                $sql2 = sprintf("SELECT * FROM zamowienia WHERE idklienta='%s' AND `status`!='wyslano'", mysqli_real_escape_string($polaczenie,$id));
                if ($rezultat2 = @$polaczenie->query($sql2)){
                    ...
                    if($ile_zamowien2>0){
                        ...
                        while ($wiersz2 = $rezultat2->fetch_object()) {
                            ...
                            $dataRealizacji = $wiersz2->data;
                            $dataRealizacji = explode("-",$dataRealizacji);
                            echo "<td>".$dataRealizacji[0]."-".$dataRealizacji[1]."-0".($dataRealizacji[2] + 3)."</td>";
                            echo "<td>".$wiersz2->status."</td>";
    ...
    echo "By korzystać ze śledzenia zamówień musisz być zalogowany";
}
