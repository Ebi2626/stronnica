...
function countingItems(){
    let tytuly = document.querySelectorAll(".fullshoppingCart__row");
        ...
        tytuly = document.querySelectorAll(".fullshoppingCart__row");
        for (let i=1; i<tytuly.length; i++){
            let item = tytuly[i];
...
function removingItems(){
    ...
    removingButtons.forEach(el => {
        ...
        let tytul = ksiozka[0];
        let autor = ksiozka[1];
        let ilosc = el.parentNode.parentNode.children[2].innerHTML;
        let Pozycja = {
            autor: autor,
            tytul: tytul,
            ilosc: ilosc
        ...
        el.addEventListener("click", function removeItem(el){
            ...
            console.log(Pozycja.ilosc);
             let PozycjaJSON = JSON.stringify(Pozycja);
            if (Pozycja.ilosc>1){
                let staraIlosc = Pozycja.ilosc;
                console.log(staraIlosc-1);
...
window.addEventListener("DOMContentLoaded", countingValue);
